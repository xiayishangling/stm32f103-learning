...
// @参数 ...   ：可变参数
//
/*以 My_USART_Printf(USART1, "电压：%.2fV\n", 3.321); 为例：
函数接收参数：串口 USART1、格式字符串 "电压：%.2fV\n"、可变参数 3.321；
初始化可变参数指针：__va_start(argptr, Format) → argptr 指向 3.321；
拼接字符串：vsprintf(format_buffer, Format, argptr) → 把 "电压：%.2fV\n" 和 3.321 拼成 "电压：3.32V\n"，存到 format_buffer 里；
释放指针：__va_end(argptr) → 清理 argptr 占用的资源；
串口发送：My_USART_SendString(USART1, format_buffer) → 把拼接好的字符串通过 USART1 发出去，串口工具就能看到结果。*/
